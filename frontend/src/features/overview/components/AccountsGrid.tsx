  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 md:gap-3">
      {accounts.slice(0, 6).map(account => {
        ...
        return (
          <div key={account.id} className="rounded-lg p-3 md:p-4 min-w-0" style={{ backgroundColor: 'var(--elev-1)' }}>
            ...
            >
              {/* The spoken form never depends on a minus sign or a colour. */}
              <span aria-hidden="true">{balance.text}</span>
              <span className="sr-only">{`${account.name}: ${balance.srText}`}</span>
            </p>

            {balance.detail && (
              <p className="text-[10px] mt-0.5 truncate" style={{ color: 'var(--dim)' }}>{balance.detail}</p>
            )}
          </div>
        );
      })}
