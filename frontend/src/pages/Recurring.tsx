...
import { getRecurring, deleteRecurring, updateRecurring, processDueRecurring, getAccounts, getCategories } from '../utils/api';
...
const Recurring: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (count > 0) {
        ...
        alert('No due transactions right now');
      }
      load();
  ...
  const RecurringItem: React.FC<{ item: RecurringTransaction }> = ({ item }) => {
    ...
    return (
      <div className="flex items-center gap-3 px-4 py-3.5 group transition-colors hover:bg-surface2"
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="text-right">
            <p className="font-mono font-bold text-sm" style={{ color: pos ? '#2ecc8a' : '#ff5f6d' }}>
              {pos ? '+' : '-'}${fmt(Math.abs(Number(item.amount)))}
            </p>
          </div>
            ...
            <div className="flex gap-2">
              {dueNow.length > 0 && (
                <button onClick={handleProcess} disabled={processing}
                  ...
                  {processing ? '…' : `Log ${dueNow.length} due`}
                </button>
              )}
          ...
          {dueNow.length > 0 && (
            <div>
              ...
              <button onClick={handleProcess} disabled={processing}
                ...
                {processing ? 'Processing…' : `Log all ${dueNow.length} due transactions`}
              </button>
            </div>
          )}
